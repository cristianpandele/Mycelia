...
TEST_CASE ("Boot performance")
{
    ...
    {
        std::vector<Catch::Benchmark::storage_for<Mycelia>> storage (size_t (meter.runs()));
    ...
    (Catch::Benchmark::Chronometer meter)
    {
        std::vector<Catch::Benchmark::destructable_object<Mycelia>> storage (size_t (meter.runs()));
        for (auto& s : storage)
            s.construct();
